...
object NMS1_16_1 : NMS {
    ...
    override fun getCommandSourceStack(sender: CommandSender): Any {
        val overworld = MinecraftServer.getServer().D()
    ...
    private fun getParticleData(particle: Particle, options: ParticleParam): ParticleData<*> = when (options) {
        ...
        is ParticleParamItem -> ParticleData(
            particle,
            CraftItemStack.asBukkitCopy(options::class.java.getDeclaredMethod("c").also { it.isAccessible = true }(null) as net.minecraft.server.v1_16_R1.ItemStack)
        )
